...
class spell_dk_wandering_plague_aura : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo &eventInfo)
    {
        ...
        if (!eventInfo.GetActor()->roll_lucky_f(eventInfo.GetActor()->GetUnitCriticalChance(BASE_ATTACK, eventInfo.GetActionTarget())))
            return false;
...
public:
    ...
    bool operator()(WorldObject *obj) const
    {
        ...
        {
            if ((target->isDead() || (_allowGhoul && target->GetEntry() == NPC_DK_GHOUL && target->GetOwnerGUID() == _casterGUID)) && !(target->GetCreatureTypeMask() & CREATURE_TYPEMASK_MECHANICAL_OR_ELEMENTAL) && target->GetDisplayId() == target->GetNativeDisplayId())
                return false;
        }
...
public:
    ...
    bool operator()(WorldObject *obj) const
    {
        if (Unit *target = obj->ToUnit())
        {
            if (!target->IsAlive() && _caster->isHonorOrXPTarget(target) && target->GetCreatureType() == CREATURE_TYPE_HUMANOID && target->GetDisplayId() == target->GetNativeDisplayId())
                return false;
        }
...
class spell_dk_spell_deflection : public AuraScript
{
    ...
    void Absorb(AuraEffect * /*aurEff*/, DamageInfo &dmgInfo, uint32 &absorbAmount)
    {
        ...
        if ((dmgInfo.GetDamageType() == SPELL_DIRECT_DAMAGE) && GetCaster()->roll_lucky_f(chance))
            absorbAmount = CalculatePct(dmgInfo.GetDamage(), absorbPct);
    }
