...
// CRAFTCRAFT
const float m_diminishing_k = 0.9560f; // Warrior
...
float Player::GetMissPercentageFromDefence() const
{
    ...
    return nondiminishing + (diminishing * miss_cap / (diminishing + miss_cap * m_diminishing_k));
}

void Player::UpdateParryPercentage()
{
    // CRAFTCRAFT STATS
    float parry_cap = 47.003525f;
    // No parry
    float value = 0.0f;
    m_realParry = 0.0f;
    if (CanParry() && parry_cap > 0.0f)
    {
        ...
        m_realParry = nondiminishing + diminishing * parry_cap / (diminishing + parry_cap * m_diminishing_k);
...
void Player::UpdateDodgePercentage()
{
    ...
    m_realDodge = nondiminishing + (diminishing * dodge_cap / (diminishing + dodge_cap * m_diminishing_k));
